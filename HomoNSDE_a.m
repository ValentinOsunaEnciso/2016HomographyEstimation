...
figure; tam1=size(X1o,2); tam2=size(X2o,2); tam=size(X1,2);
...
figure,plot(Pt(:,d + 1),Pt(:,d + 2),'g*'); xlabel('rank'),ylabel('inliers');
...
%     Qt=GA_a(parents,M,d,mu,mum,min_range,max_range,X);
    Qt=DE_a(Pt, M, d, min_range, max_range,X);
    ...
    if ~mod(i,5)
      ...
      %[temp,~]=sort(Pt(:,d + 2)); Pt(:,d + 2)=temp;
      plot(Pt(:,d + 1),Pt(:,d + 2),'db','LineWidth',3); xlabel('B','FontSize',20),ylabel('\epsilon','FontSize',20);
...
% figure,imshow(K1,[]),title(strcat('CORRELACIÓN CRUZADA',num2str(NK1))),
% figure,imshow(K2,[]),title(strcat('CORRELACIÓN CRUZADA',num2str(NK2))),
% figure,imshow(K3,[]),title(strcat('CORRELACIÓN CRUZADA',num2str(NK3))),
h1=figure;
imshow(K1,[]);
